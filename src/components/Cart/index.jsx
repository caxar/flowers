...
import { clearProductItems } from "../../redux/cart/cartSlice";
import { getDiscountPrice, getTotalPriceDiscount } from "../../utils/help";
...
const Cart = () => {
  ...
  const totalPrice = items.reduce(
    (sum, item) => item.price.newPrice * item.count + sum,
    0
  );

  const totalDiscountPrice = items.reduce(
    (sum, item) => item.price.oldPrice * item.count + sum,
    0
  );
      ...
      <div className="container">
        {items.length == 0 ? (
          <div className="cart-empty">
            <div className="cart-empty_title">Ой, в вашей корзине пусто!</div>
            <div className="cart-empty_img">
              <img src={cartEmpty} alt="empty-cart" />
            </div>
            <div className="cart-empty_text">
              <p>
                Перейдите на главную старницу и выберите понравившийся товар.
              </p>
            </div>
          ...
          <>
            <div className="cart-block">
              ...
              <div
                ...
                onClick={() => dispatch(clearProductItems())}
              >
                Очистить корзину
                <svg
                  width="20px"
            ...
            <div className="cart-wrapper">
              <div className="cart-wrapper__order">
                {items?.map((item) => (
                  <CartItem key={item.id} {...item} />
                ))}
              </div>
              <div className="cart-wrapper__total">
                <span></span>
                <ul>
                  <li>
                    <span className="price-title">Итого к оплате:</span>
                  ...
                  <li>
                    ...
                    <span className="price">
                      {(totalPrice - totalDiscountPrice).toLocaleString(
                        "ru-RU"
                      )}{" "}
                      &#8381;
                    </span>
